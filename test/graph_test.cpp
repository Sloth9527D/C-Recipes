...
TEST(TypeList, TLtest)
{
    ...
    static_assert(
        std::is_same_v<Alist::exportTo<std::tuple>, std::tuple<int, char>>);
    static_assert(
        std::is_same_v<Alist::exportTo<std::variant>, std::variant<int, char>>);
}
...
TEST(TypeList, Func)
{
    ...
    static_assert(Fold_t<LongList, std::integral_constant<size_t, 0>,
                         TypeSizeAcc>::value == 18);
    static_assert(std::is_same_v<Concat_t<TypeList<int, char>, TypeList<float>>,
                                 TypeList<int, char, float>>);
    static_assert(Elem_v<LongList, char>);
    static_assert(
        std::is_same_v<Unique_t<LongList>, TypeList<char, float, double, int>>);
    static_assert(std::is_same_v<Partition_t<LongList, SizeLess4>::Satisfied,
                                 TypeList<char, char>>);
    static_assert(std::is_same_v<Partition_t<LongList, SizeLess4>::Rest,
